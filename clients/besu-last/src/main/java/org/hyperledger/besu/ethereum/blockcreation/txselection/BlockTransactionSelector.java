 ...
 *
 * --- LAST PATCH ---
 * Adds Locality-Aware Scheduling of Transactions (LAST) to block construction.
 * Activated via JVM system property -Dlast.variant=<VARIANT>
 * Variants: DISABLED (default), ADDRESS_LOCALITY, WORKING_SET_AFFINITY, HYBRID_FEE_LOCALITY
 * Config:   -Dlast.alpha=0.9 -Dlast.beta=0.1 (only for HYBRID_FEE_LOCALITY)
 *           -Dlast.log.path=/path/to/last_metrics.csv (optional per-block CSV logging)
 */
...
public class BlockTransactionSelector {
  ...
  // ── LAST fields ─────────────────────────────────────────────────────────────
  //
  // The tracker MUST persist across block-construction cycles so that inter-block
  // warm state is preserved.  BlockTransactionSelector is re-instantiated for
  // every block, so a per-instance tracker would always start cold.  We use a
  // JVM-level static singleton instead; block creation is single-threaded in Besu
  // so no additional synchronisation is needed.
  private static final StateLocalityTracker LAST_TRACKER_SINGLETON = new StateLocalityTracker();
  private final StateLocalityTracker lastTracker;
  private final LASTScheduler lastScheduler;
  private final LASTMetricsLogger lastLogger;   // null when logging is disabled
  ...
  public BlockTransactionSelector(
    ...
    // ── LAST init ────────────────────────────────────────────────────────────
    final String variantName = System.getProperty("last.variant", "DISABLED");
    LASTScheduler.Variant resolvedVariant;
    try {
      resolvedVariant = LASTScheduler.Variant.valueOf(variantName);
    } catch (IllegalArgumentException e) {
      LOG.warn("LAST: unknown variant '{}', falling back to DISABLED", variantName);
      resolvedVariant = LASTScheduler.Variant.DISABLED;
    }
    final double lastAlpha = parseDouble(System.getProperty("last.alpha"), 0.5);
    final double lastBeta  = parseDouble(System.getProperty("last.beta"),  0.5);
    this.lastTracker   = LAST_TRACKER_SINGLETON;   // shared singleton — persists across blocks
    this.lastScheduler = new LASTScheduler(resolvedVariant, lastTracker, lastAlpha, lastBeta);

    LASTMetricsLogger tmpLogger = null;
    final String logPath = System.getProperty("last.log.path");
    if (logPath != null) {
      try {
        tmpLogger = new LASTMetricsLogger(logPath, lastScheduler, lastTracker);
      } catch (java.io.IOException ex) {
        LOG.warn("LAST: could not open metrics log at '{}': {}", logPath, ex.getMessage());
      }
    }
    this.lastLogger = tmpLogger;

    if (resolvedVariant != LASTScheduler.Variant.DISABLED) {
      LOG.info("LAST: variant={} alpha={} beta={} log={}",
          resolvedVariant, lastAlpha, lastBeta, logPath != null ? logPath : "(disabled)");
    }
    // ────────────────────────────────────────────────────────────────────────
  }

  private static double parseDouble(final String s, final double def) {
  ...
  public TransactionSelectionResults buildTransactionListForBlock() {
    ...
    // ── LAST: log per-block metrics ──────────────────────────────────────────
    if (lastLogger != null && blockStartNanos > 0) {
      final long blockDurationMs =
          TimeUnit.NANOSECONDS.toMillis(System.nanoTime() - blockStartNanos);
      final int txCount = transactionSelectionResults.getSelectedTransactions().size();
      try {
        lastLogger.logBlock(
            blockSelectionContext.processableBlockHeader().getNumber(),
            txCount,
            blockDurationMs);
      } catch (Exception ex) {
        LOG.warn("LAST: failed to write metrics row: {}", ex.getMessage());
      }
    }
    // ────────────────────────────────────────────────────────────────────────
  ...
  private void timeLimitedSelection() {
    // ── LAST: reorder candidates before selection ────────────────────────────
    if (lastScheduler.getVariant() != LASTScheduler.Variant.DISABLED) {
      final var txSelection = ethScheduler.scheduleBlockCreationTask(() -> {
        blockStartNanos = System.nanoTime();
        lastTracker.reset();

        // Snapshot all pending transactions from the pool
        final List<PendingTransaction> candidates =
            new ArrayList<>(blockSelectionContext.transactionPool().getPendingTransactions());

        // Reorder by LAST policy
        final List<PendingTransaction> ordered = lastScheduler.reorder(
            candidates,
            blockSelectionContext.processableBlockHeader().getBaseFee());

        // Evaluate in LAST order, respecting stop() and cancellation
        for (final PendingTransaction candidateTx : ordered) {
          if (isCancelled.get()) break;
          final var result = evaluateTransaction(candidateTx);
      ...
      try {
        ...
        if (isCancelled.get()) {
          throw new CancellationException("Cancelled during LAST transaction selection");
        }
        LOG.warn("Error during LAST block transaction selection", e);
      } catch (TimeoutException e) {
        synchronized (isTimeout) {
          isTimeout.set(true);
        }
        LOG.warn(
            "LAST: interrupting transaction selection after max {}ms",
            blockTxsSelectionMaxTime, e);
      }
      return;
    }
    // ────────────────────────────────────────────────────────────────────────

    // DISABLED path: original fee-priority selection via the pool's own ordering
    blockStartNanos = System.nanoTime();
  ...
  private TransactionSelectionResult handleTransactionSelected(
    ...
    // ── LAST: record warm-state access for successfully included transaction ──
    if (lastScheduler.getVariant() != LASTScheduler.Variant.DISABLED) {
      final Set<Address> touched = new HashSet<>();
      transaction.getTo().ifPresent(to -> touched.add((Address) to));
      touched.add(transaction.getSender());
