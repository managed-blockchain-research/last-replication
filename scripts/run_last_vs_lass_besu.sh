#!/bin/bash
# ============================================================
# LAST vs LASS Head-to-Head Evaluation — Hyperledger Besu @ 1GB Heap
#
...
# Output: results/last_vs_lass/<RUN_ID>/
# ============================================================
set -e
...
# ── Output directory ──────────────────────────────────────────────────────────
RUN_ID=$(date +%Y%m%d_%H%M%S)_last_vs_lass
RESULTS_DIR="/home/yeochan.yoon/caliper-stress-test/results/last_vs_lass/${RUN_ID}"
mkdir -p "${RESULTS_DIR}"
...
LAST vs LASS Head-to-Head Evaluation @ 1GB Heap
================================================
Run ID: ${RUN_ID}
Date:   $(date)
Host:   $(hostname)
...
Binaries:
  all variants: ${BESU_LASS} (commit: ${BESU_SRC_COMMIT})
  Note: besu-source used for all variants (eliminates besu-24.1.1 WebSocket deadlock bug)

LAST patch: ADDRESS_LOCALITY via -Dlast.variant
...
Variants:
  baseline   : besu-source, DISABLED LAST, no LASS
  lass75     : besu-source, DISABLED LAST, LASS-75
  last_al    : besu-source, ADDRESS_LOCALITY, no LASS
...
python3 scripts/analyze_last_vs_lass.py "${RESULTS_DIR}" \
    > "${RESULTS_DIR}/analysis.log" 2>&1 && \
    echo "Report: ${RESULTS_DIR}/final_LAST_vs_LASS_1GB_Evaluation.md" || \
