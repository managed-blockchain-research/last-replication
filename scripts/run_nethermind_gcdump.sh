...
OUT_DIR="${ROOT}/results/nethermind/gcdump-${TS}"
...
collect_gcdump() {
  name="$1"
  pid="$2"
  dump_path="${OUT_DIR}/${name}_gcdump.gcdump"
  report_path="${OUT_DIR}/${name}_gcdump_report.txt"
  echo "==> Collecting gcdump (${name})..."
  dotnet-gcdump collect -p "${pid}" -o "${dump_path}" >/dev/null 2>&1
  dotnet-gcdump report "${dump_path}" > "${report_path}"
}
...
echo "✅ Nethermind gcdump run finished."
