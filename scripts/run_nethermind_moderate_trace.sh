...
OUT_DIR="${ROOT}/results/nethermind/moderate-trace-${TS}"
...
run_case() {
  ...
  echo "==> dotnet-trace collect (${name})..."
  dotnet-trace collect \
    --process-id "${pid}" \
    --providers "System.Runtime" \
    --duration 00:10:00 \
    --output "${OUT_DIR}/${name}_runtime.nettrace" \
    > "${OUT_DIR}/${name}_trace.log" 2>&1 &
  TRACE_PID=$!
...
echo "✅ Nethermind moderate-trace run finished."
