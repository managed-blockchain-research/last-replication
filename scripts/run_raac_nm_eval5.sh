#!/usr/bin/env bash
# Interleaved baseline + RAAC NM runs to reach n=10 each.
# Baseline (no AI filter): +5 runs | RAAC (serve.py filter): +4 runs
set -euo pipefail
cd /home/yeochan.yoon/caliper-stress-test

RUN_ID="$(date +%Y%m%d_%H%M%S)_raac_nm_eval5"
...
echo "======================================================================"
echo "RAAC NM eval5 (interleaved) | RUN_ID: ${RUN_ID}"
echo "======================================================================"
...
ensure_ai_service() {
    ...
    while [ $c -lt 20 ]; do
        result=$(curl -s --max-time 3 -X POST http://127.0.0.1:8000/predict \
            ...
            -d '{"tx_hash":"check","gas_price":1,"gas_limit":12000000,"wei_value":0,"bytecode_size":24000,"opcode_count":40000}' 2>/dev/null)
...
# ── Interleaved execution: (raac_1, baseline_1), (raac_2, baseline_2), ... ──
echo ""
echo "=============================="; echo "Interleaved: 4×RAAC + 5×BASELINE"; echo "=============================="
...
# 5th baseline (one extra to match n=10)
sleep 15
run_nm_single "baseline" "5" "${BENCHCONFIG_BASELINE}" || echo "  WARNING: baseline_nm_5 failed"

echo ""
echo "======================================================================"
echo "eval5 COMPLETE — results in ${RESULTS_DIR}"
echo "======================================================================"
