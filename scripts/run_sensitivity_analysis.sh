#!/bin/bash
# ============================================================
# LASS Sensitivity Analysis: Baseline + LASS-60 + LASS-75 + LASS-90
#
# Provenance:
#   - LASS binary: /home/yeochan.yoon/besu-source/build/install/besu/bin/besu
#   - LASS commit: 9b0e38fa (release-24.1.1 + ESpill fixes)
#   - Baseline binary: /home/yeochan.yoon/besu-24.1.1/bin/besu (unmodified)
#   - Heap: 2.5GB (sized so peak usage ~2.4GB hits all 3 thresholds)
#   - Workload: extreme_baseline_test.py, 20 workers, 600s, 200 slots/tx
#   - Run date: $(date +%Y-%m-%d)
# ============================================================
...
echo "======================================================================"
echo "LASS Sensitivity Analysis"
echo "Run ID: ${RUN_ID}"
echo "Heap: ${HEAP_SIZE} | Workers: ${WORKLOAD_WORKERS} | Duration: ${WORKLOAD_DURATION}s | Slots: ${WORKLOAD_SLOTS}"
...
# ─────────────────────────────────────────
# Helper: deploy contract and return address
# ─────────────────────────────────────────
deploy_contract() {
    local addr
    addr=$(python3 deploy_contract.py 2>&1 | grep "Contract Address:" | awk '{print $3}')
    echo "${addr}"
}

# ─────────────────────────────────────────
# Helper: run one experiment
#   $1 = variant name (baseline / lass60 / lass75 / lass90)
#   $2 = besu binary path
#   $3 = extra JVM opts for LASS thresholds (empty for baseline)
# ─────────────────────────────────────────
run_experiment() {
    local variant="$1"
    local besu_bin="$2"
    ...
    local data_dir="/home/yeochan.yoon/caliper-stress-test/data_sensitivity_${variant}"
    local gc_log="${RESULTS_SUBDIR}/gc_${variant}.log"
    local pid_file="/tmp/besu_sensitivity.pid"
...
-Dcom.sun.management.jmxremote.ssl=false"
    ...
    if ! kill -0 ${besu_pid} 2>/dev/null; then
        echo "  ERROR: Besu died. Check: tail -30 ${data_dir}/besu_console.log"
    ...
    # Copy besu console log for reference
    cp "${data_dir}/besu_console.log" "${output_prefix}_besu_console.log"

    echo "  Saved to: ${output_prefix}_*"
    echo "  ✓ ${variant} complete"
}

# ─────────────────────────────────────────
# Check no Besu already running
# ─────────────────────────────────────────
...
LASS Sensitivity Analysis Provenance
=====================================
Run ID: ${RUN_ID}
Date: $(date)
Host: $(hostname)
...
Binaries:
  Baseline: /home/yeochan.yoon/besu-24.1.1/bin/besu (stock Besu 24.1.1, no LASS)
  LASS:     /home/yeochan.yoon/besu-source/build/install/besu/bin/besu
  Commit:   ${LASS_COMMIT}

JVM:
  Heap:     ${HEAP_SIZE}
  GC:       G1GC -XX:MaxGCPauseMillis=200 -XX:G1HeapRegionSize=4m -XX:G1HeapWastePercent=5
...
LASS Thresholds:
  lass60:   activate=60%, deactivate=45%, consecutive=3
  lass75:   activate=75%, deactivate=60%, consecutive=3
  lass90:   activate=90%, deactivate=75%, consecutive=3
...
run_experiment "lass60" "${BESU_LASS_BIN}" \
    "-Dlass.old.gen.activation.threshold=0.60 -Dlass.old.gen.deactivation.threshold=0.45 -Dlass.old.gen.consecutive.samples=3"

run_experiment "lass75" "${BESU_LASS_BIN}" \
    "-Dlass.old.gen.activation.threshold=0.75 -Dlass.old.gen.deactivation.threshold=0.60 -Dlass.old.gen.consecutive.samples=3"

run_experiment "lass90" "${BESU_LASS_BIN}" \
    "-Dlass.old.gen.activation.threshold=0.90 -Dlass.old.gen.deactivation.threshold=0.75 -Dlass.old.gen.consecutive.samples=3"
...
echo "======================================================================"
echo ""
echo "Next: python3 scripts/analyze_sensitivity.py ${RESULTS_SUBDIR}"
